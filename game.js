...
const characters = [
  ...
  {
    name: "Track",
    weapons: [{ name: "gauntlet", maxDamage: 30 }],
    health: 100,
  },
];
...
while (true) {
  ...
  while (enemy.health > 0 && character.health > 0) {
    ...
    console.log(
      `- Enemy "${enemy.name}" wurde schwer getroffen (Damage: ${weaponHero.maxDamage}). Neue Health: ${enemy.health}`
    );
    console.log();
    ...
    console.log(
      `- Du wurdest schwer getroffen (Damage: ${weaponEnemy.maxDamage}). Neue Health: ${character.health}`
    );
    console.log();
  }
  ...
  if (character.health < 0) {
    console.log(
      "Du hast es leider nicht geschafft! Aber du warst immer bemüht..."
    );
    break;
  }
  // enemy tot
